		...
		* client will hang for a while, until it timeouts. */
		...
		if (webcache.containsKey(request.URI)) {
			...
			try {
				...
				String cached = webcache.get(request.URI);

				/* COMPLETE ME: send the cached web object to the client */
                toClient.writeBytes(cached);
			...
			try {
				...
				// toClient.write (response.headers.getBytes(StandardCharsets.UTF_8));
	...
	public static void main(String args[]) {
		...
		while (true) {
			try {
				...
				/* COMPLETE ME: accept an incoming request to the socket */;
				System.out.println("");
				System.out.println("Got connection " + client);
				handle(client);
					// System.out.println("Double " + client);
