...
public class ProxyCache {
	...
	port = p;
		try {
			socket =  new ServerSocket(8081)/* COMPLETE ME: create a ServerSocket for communication with a client */;

            /* COMPLETE ME: create a HashMap for storing caches */
			webcache =  new HashMap<String, String>();
		} catch (IOException e) {
		...
		* client will hang for a while, until it timeouts. */
		...
		if (webcache.containsKey(request.URI)) {
			...
			try {
				...
				server = /* COMPLETE ME: create a new socket for connection to the origin server */;
				DataOutputStream toServer = /* COMPLETE ME: create a DataOutputStream object for sending the request */;
			...
			try {
				DataInputStream fromServer = /* COMPLETE ME: use a DataInputStream object to read the response from the server socket*/;
				response = /* COMPLETE ME: create an HttpResponse object, check HttpResponse.java for details on the HttpResponse class */;
				System.out.println("Proxy receives the response from Origin server for: "+ request.URI);
				
				DataOutputStream toClient = /* COMPLETE ME: create a DataOutputStream object for forwarding the response to the client */;
				
				/* Write response to the client. First headers, then body */
				/* COMPLETE ME: write the response header to the socket */
				/* COMPLETE ME: write the response body to the socket */
    ...
    public static void main(String args[]) {
		...
		while (true) {
			try {
				client = /* COMPLETE ME: accept an incoming request to the socket */;
				System.out.println("");
